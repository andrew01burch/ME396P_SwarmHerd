...
def train_model(model, replay_buffer, batch_size, gamma):
    ...
    for state, action, reward, next_state, done in minibatch:
        ...
        model.fit(state.reshape(1, -1), target_f, epochs=1, verbose=0)  # Set verbose to 0
...
while running:
    ...
    # Check if the simulation should reset
    if done:
        current_duration = pygame.time.get_ticks() - start_time
        if current_duration <= max_duration:
            consecutive_successes += 1
            print(f"Training iteration completed successfully in {current_duration} ms.")
        else:
            consecutive_successes = 0  # Reset the count if the run was not successful within 10 seconds
